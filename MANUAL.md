...
The menu strip participates in a unified window visibility management system alongside the command palette and task panel. When all three components are present in a scene and `window_presentation` is provided, they share synchronized state and consistently respect the `window_menu_opt_in` opt-out flag. The three components are completely independent and optional—any combination can be omitted without affecting the others:

- **Menu Strip**: Displays windows in a dynamic Windows submenu when `WindowMenuOptions.shown=True`. Automatically filters by `window_menu_opt_in` when `window_presentation` is available; shows all windows otherwise.
...
Setting `window_menu_opt_in=False` on any window spec (WindowSpec, AnchoredWindowSpec, or FeatureWindowBundleBindingSpec) excludes that window from **all three** management systems. By default, windows are included (`window_menu_opt_in=True`). This opt-out pattern ensures that whichever components are present in a scene will all show the same set of manageable windows. When `window_presentation` is provided to `MenuStripControl` (or its helper functions), opt-in filtering is automatically applied.
