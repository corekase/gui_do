from __future__ import annotations

from typing import Any, Callable, Optional

from ..geometry import point_in_rect


def topmost_window_at_mouse(gui: Any) -> Optional[Any]:
    """Return the topmost visible window currently under the mouse position."""
    mouse_pos = gui._get_mouse_pos()
    for candidate in tuple(gui.windows)[::-1]:
        if not candidate.visible:
            continue
        if point_in_rect(mouse_pos, candidate.get_window_rect()):
            return candidate
    return None


def drag_blocked_by_overlay(gui: Any, owner_window: Optional[Any]) -> bool:
    """Return True when drag enters a region occluded by another window."""
    topmost_window = topmost_window_at_mouse(gui)
    if owner_window is None:
        return topmost_window is not None
    return topmost_window is not None and topmost_window is not owner_window


def cancel_drag_for_overlay_contact(
    gui: Any,
    is_dragging: bool,
    owner_window: Optional[Any],
    cancel_drag: Callable[[], None],
    on_cancel: Optional[Callable[[], None]] = None,
) -> bool:
    """Cancel drag when pointer enters an overlaid window region."""
    if not is_dragging:
        return False
    if not drag_blocked_by_overlay(gui, owner_window):
        return False
    cancel_drag()
    if on_cancel is not None:
        on_cancel()
    return True
