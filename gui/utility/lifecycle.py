...
class LifecycleCoordinator:
    ...
    def run_postamble(self) -> None:
        ...
        self.gui._screen_postamble()

    def run_preamble(self) -> None:
        self.gui._screen_preamble()
        for window in self.gui.windows:
            if window.visible:
                window.run_preamble()
        if self.gui.task_panel is not None and self.gui.task_panel.visible:
    ...
    def set_screen_lifecycle(
        ...
        self.gui._screen_preamble = preamble if preamble is not None else (lambda: None)
        self.gui._screen_event_handler = event_handler if event_handler is not None else (lambda _event: None)
        self.gui._screen_postamble = postamble if postamble is not None else (lambda: None)
