...
# Manual Step 5 — Main Systems Reference (Systems 8.5–8.8) (Discovery-Based)

## Scope

This step is responsible for generating or updating the Main Systems Reference chapters 8.5–8.8 in MANUAL.md. All content must be generated by discovering the current state of the codebase, tests, demo features, and docs at the time of invocation. No API names, specs, or patterns are assumed—everything is verified live.

## Operations

1. **Discovery Pass:**
  - Read MANUAL.md to find the Main Systems Reference (8.5–8.8) sections to replace.
  - Read `gui_do/__init__.py` (Tiers 4, 8, 12, 13, 21, 28, 29 and all relevant tiers) to discover current API names and patterns.
  - Read relevant control, layout, focus, and overlay files for implementation and usage patterns.
  - Read docs for any new system requirements.
2. **Write/Update:**
  - Replace the Main Systems Reference (8.5–8.8) sections with comprehensive, up-to-date content, using the standard chapter template (what/why, mental model, APIs, usage, examples, advanced patterns, mistakes, cross-links, back-to-ToC).
...
- If MANUAL.md exists, replace only the Main Systems Reference (8.5–8.8) sections. Remove obsolete information.
- If it does not exist, this step is run after the previous Main Systems Reference step.
