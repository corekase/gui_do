...
# Manual Step 2 — Conceptual Foundations (Theory) (Discovery-Based)

## Scope

This step is responsible for generating or updating the Conceptual Foundations (Theory) section of MANUAL.md. All content must be generated by discovering the current state of the codebase, tests, demo features, and docs at the time of invocation. No API names, specs, or patterns are assumed—everything is verified live.

## Operations

1. **Discovery Pass:**
  - Read MANUAL.md to find the Conceptual Foundations (Theory) section to replace.
  - Read `gui_do/__init__.py` (Tier 1, Tier 3, and all relevant tiers) to discover current spec and observable type names.
  - Read `gui_do/features/data_driven_runtime.py` and `gui_do/features/feature_lifecycle.py` for spec pipeline and lifecycle phases.
  - Skim demo_features for concrete lifecycle examples.
  - Read docs for any new theory or runtime facility requirements.
2. **Write/Update:**
  - Replace the Conceptual Foundations (Theory) section with comprehensive, verbose prose, covering all required subsections.
  - Explicitly cover all higher-level runtime faculties as discovered in the codebase, explaining their purpose, declarative composition, and lifecycle role.
  ...
  - Remove obsolete or outdated information from this section.
3. **Verification:**
  - Ensure the section is non-empty, up to date, and covers all required theory and runtime faculties.
  - Ensure all code examples are valid and verified.
  - Remove any obsolete or outdated information from this section.

## Update/Removal Logic

- If MANUAL.md exists, replace only the Conceptual Foundations (Theory) section. Remove obsolete information.
- If it does not exist, this step is run after the skeleton is created.

## Sequential Execution

This step must be executed after the skeleton/preamble step and completed before any downstream subprompt runs.
