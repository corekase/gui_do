...
# Manual Step 8 — Testing, Diagnostics, Performance, Migration, FAQ, Appendices (Discovery-Based)

## Scope

This step is responsible for generating or updating the Testing, Diagnostics, Performance, Migration, FAQ, and Appendices sections in MANUAL.md. All content must be generated by discovering the current state of the codebase, tests, demo features, and docs at the time of invocation. No API names, specs, or patterns are assumed—everything is verified live.

## Operations

1. **Discovery Pass:**
   - Read MANUAL.md to find the Testing, Diagnostics, Performance, Migration, FAQ, and Appendices sections to replace.
   - Read `docs/runtime_operating_contracts.md` for restore report fields, stability policy, and scheduler budget values.
   - List the `tests/` directory for contract and runtime test files.
   - Read `gui_do/__init__.py` (Tiers 7, 16, 17, 32 and all relevant tiers) to discover current API names and patterns.
   - Read all tier sections from `gui_do/__init__.py` for the Appendix D.1 Tier-to-System matrix and Appendix D Quick Index.
2. **Write/Update:**
   - Replace the Testing, Diagnostics, Performance, Migration, FAQ, and Appendices sections with comprehensive, up-to-date content, including all required checklists, quick indexes, and matrices.
...
- If MANUAL.md exists, replace only the Testing, Diagnostics, Performance, Migration, FAQ, and Appendices sections. Remove obsolete information.
- If it does not exist, this step is run after the previous steps.

## Sequential Execution

This step must be executed after the previous steps and completed before the enrichment pass.
